...
ABC241 Exからは、上限付き等比級数積の巨大次数係数を返す
`bounded_geometric_product_coefficient`を追加した。分子を部分集合列挙し、
相異なる一次分母を部分分数分解することで、上限や次数に比例せず計算する。
ABC241 Gからは、既知結果を保って指定選手を単独優勝させる
`complete_roundrobin_for_unique_winner`と、可能な全選手を列挙するAPIを追加した。
