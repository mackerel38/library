...
- `incrementalreachabilityresult`: 頂点有効化に伴う到達情報。reached_at[v]は初めて到達するstep、count[i]はstep i後の到達数。
- `incremental_reachability`: O(n+m)。order順に頂点を有効化した各時点の、sourcesからの有効頂点内到達集合を返す。
