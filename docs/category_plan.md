...
- ABC244 Ex: 点追加と任意方向の内積最大化を扱うincremental convex hullとして実装済み。
- ABC244 G: 頂点訪問回数の偶奇を指定する短いwalk構成として実装済み。
- ABC243 G: 平方根遷移列を$O(X^{1/4})$前計算・$O(1)$クエリで数えるAPIとして実装済み。
- ABC243 Ex: 長方形4近傍グリッドの最小頂点separatorと最適集合数として実装済み。
- ABC242 G: 既存の`Mo`と使用verifyで処理でき、問題専用APIは追加しない。
- ABC242 Ex: 区間部分集合の全域被覆数と、ランダム選択による被覆完了期待値として実装済み。
- ABC241 Ex: 上限付き等比級数積の巨大次数係数として実装済み。
- ABC241 G: 未確定試合の勝者割当てと勝数上限を扱うflow APIとして実装済み。
- ABC240 G: 1・2・3次元の軸方向単位格子walk数え上げとして実装済み。
- ABC240 Ex: 位置順・辞書順とも真に増加する非重複substring列の最大長として実装済み。
- ABC239 Ex以降: H/Exを一問ずつ監査し、同じ回のG問題も続けて確認する。
