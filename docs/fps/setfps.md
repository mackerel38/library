...
- `setfps_exp`: O(3^n)。集合冪級数f（f[0]=0）のexpを返す。配列長は2冪。
- `setfps_log`: O(3^n)。集合冪級数f（f[0]=1）のlogを返す。配列長は2冪。
- `setfps_articulation_transform`: O(3^(n-1))。vertexを含む係数だけを、その偏微分SPSのexpまたはlogで置き換える。
