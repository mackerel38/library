...
`verify/atcoder_abc422_g.cpp`で公式sampleを確認済み。judge ACは未確認。
...
`verify/atcoder_abc303_h.cpp`で公式sample 3件を確認済み。judge ACは未確認。

[AtCoder ABC297 Ex - Diff Adjacent](https://atcoder.jp/contests/abc297/tasks/abc297_h)
では重み1から`N`の記号を渡し、`length_sum[N]`を使う。
`verify/atcoder_abc297_h.cpp`で公式sample 3件を確認済み。judge ACは未確認。
