...
# OGF・EGFによるグループ分配
  ...
  `poe::labeled_color_change_counts`
...
色別個数が決まった区別可能な要素の順列について、隣り合う要素の色が変わる回数の分布も求められる。
...
- `labeled_color_change_counts`: 色別個数を $m_1,\ldots,m_c$ とすると、返り値の第 $k$ 項は、隣接色が変わる場所をちょうど $k$ 個持つラベル付き順列の個数である。
...
```cpp
auto count = poe::labeled_color_change_counts<998244353>({2, 1});
// count = {0, 4, 2}
```

色0の区別された要素が2個、色1の区別された要素が1個ある場合、隣接色変化が1回の順列は4個、
2回の順列は2個である。一般に $n=\sum_i m_i$ として、`count[k]`は

$$
\#\left\{P\in S_n\mid
\left|\{j\mid \operatorname{color}(P_j)\ne\operatorname{color}(P_{j+1})\}\right|=k
\right\}
$$

を返す。各要素は同じ色でも区別される。時間計算量は $O(n+M(n)\log c)$、空間計算量は $O(n)$。
空の要素集合では、空順列一つを`{1}`として返す。
...
### `labeled_color_change_counts`

```cpp
template<int mod> fps<mod> labeled_color_change_counts(const std::vector<int>& color_counts)
```
...
[AtCoder ABC260 Ex - Colorfulness](https://atcoder.jp/contests/abc260/tasks/abc260_h)では、
入力された色の頻度を渡し、隣接色変化数ごとのラベル付き順列数を求める。
`verify/atcoder_abc260_h.cpp`に提出用コードを収録している。
