...
`check`は191個のheaderページについて、Front Matter、include、公開symbol、APIリファレンス、
実在問題例または明示的な省略を検査する。

人が読む説明は、原則として次の順番で書く。

1. `概要`: 初見でも用途を判断できる平易な説明
2. `厳密な定義`: 入出力、数式、添字範囲、成立条件を一意に定める説明
3. `Include`
4. `最短の使用例`: コンテスト中にそのまま写せる短いコード
5. 型・APIの選び方
6. 注意点、計算量、実在問題、検証状況

一つのheaderに複数の型・関数群がある場合は、冒頭に選択表を置く。手書き説明と重複する完全な
API一覧は、`<!-- API REFERENCE: COLLAPSED -->`を置いたpageでは折り畳んで生成する。
...
JekyllでHTML化してGitHub Pagesへdeployする。`verify/`以下は、oj-verifyの検証file命名である
`*.test.cpp`だけをsite生成対象に含める。それ以外の提出用`*.cpp`は含めない。
...
- docsには実在する公開問題の使用例を書く欄を生成する。確認済みの結果だけを記載する。
