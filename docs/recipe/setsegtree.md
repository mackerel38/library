...
documentation_of: //cp/recipe/setsegtree.hpp
---
...
recipe。setsegtree tree(n); tree.add(l,r,x); tree.erase(l,r,x); 0<=x<64。
...
- `tests/property/recipe/setsegtree.cpp`: ランダム区間更新・queryを集合配列の愚直解と比較
- `tests/api/recipe/setsegtree.cpp`: add・erase・添え字アクセス
