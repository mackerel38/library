---
title: "末尾追加できる閾値写像列"
documentation_of: //cp/recipe/thresholdsequence.hpp
---
...
recipe。thresholdsequence<long long> seq(up, down); seq.append(p); seq.apply(x)。追加ならしO(log n)、適用O(log^2 n)。
...
- `tests/api/recipe/thresholdsequence.cpp`: 空列、基本呼び出し、sample相当
- `tests/property/recipe/thresholdsequence.cpp`: ランダムな追加途中の全初期値を愚直走査と比較
