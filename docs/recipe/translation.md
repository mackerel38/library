...
documentation_of: //cp/recipe/translation.hpp
---
...
- `minimum_safe_translation_clear_steps`: O(h^3 w^3 K(F+1))。点集合を同時移動し、禁止点を踏まず盤外へ全消去する最小回数を返す。
...
recipe。O(h^3 w^3 K(F+1))。点集合を同時移動し、禁止点を踏まず盤外へ全消去する最小回数を返す。
...
`tests/property/recipe/translation.cpp`では、小さい盤面の点集合bit mask BFSと比較する。
