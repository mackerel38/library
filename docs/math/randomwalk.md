...
独立な一次元ランダムウォークが同一点へ集まる確率列と、加算または乗算を繰り返す確率過程の
停止時間の期待値を扱う。状態$i$から上方向へは$i+1$までしか進まないMarkov連鎖の
ポテンシャル方程式も、状態番号順に解ける。

## 厳密な定義

### `uniform_multiplicative_stopping_expectation`

独立に一様分布から整数 $X_i\in\{1,2,\ldots,N\}$ を選び、積が上限 $M$ を超えるまで
$X_1,X_2,\ldots$ を掛ける。停止時刻

$$
\tau=\min\left\{k\geq 1:\prod_{i=1}^{k}X_i>M\right\}
$$
...
using mint1000000007 = poe::staticmodint<1000000007>;
auto rolls = poe::uniform_multiplicative_stopping_expectation<mint1000000007>(faces, limit);
...
`faces >= 2`、`limit >= 0` が必要である。返り値の型 `T` は整数から構築でき、四則演算を持つ体の要素とする。
法上の期待値を求める場合は `faces - 1` が法上で逆元を持たなければならない。

$f(x)$ を「積をあと $x$ 倍まで増やしてよい状態から停止するまでの期待回数」とする。目1による自己遷移を移項すると、

$$
f(x)=\frac{N+\sum_{i=2}^{N}f(\lfloor x/i\rfloor)}{N-1},\qquad f(0)=0
$$

となる。$\lfloor x/i\rfloor$ が等しい区間をまとめ、$M$ から到達する商だけをメモ化する。
時間計算量は $O(M^{3/4})$、使用メモリは $O(\sqrt M)$。
...
```cpp
template<class T> T uniform_multiplicative_stopping_expectation(long long faces, long long limit)
```
...
[AtCoder ABC239 Ex - Dice Product 2](https://atcoder.jp/contests/abc239/tasks/abc239_h)では、
`faces=N`、`limit=M`をそのまま渡してサイコロを振る回数の期待値を求める。
...
`verify/atcoder_abc239_h.cpp`に提出用コードを収録している。
...
- `tests/property/math/randomwalk.cpp`: 乗法過程の全状態DP、少人数walkの直接分布DP、小周期の全剰余連立方程式、skip-free方程式のGauss消去との比較
