...
非互いに素な法にも対応するbaby-step giant-step離散対数を扱う。

## 厳密な定義

- `discrete_log`: O(sqrt(mod))時間・領域。discrete_log(a,b,mod): a^x=b(mod mod)となる最小x>=0。なければ-1。
...
一次変換で数列を等比数列へ移して離散対数を求める。小さい法について全探索と照合済み。
