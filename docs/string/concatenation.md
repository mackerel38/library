...
- `concatenation_order`: O(sum|s| log n)。全文字列の連結が辞書順最小になる添字順を返す。
- `second_concatenation`: O(sum|s| log n)。全順列の連結を重複込みで辞書順に並べた2番目を返す。要素数2以上。
- `minimum_repeated_concatenation`: O(Lm+km^2+|answer|)。stringsから重複を許してちょうどk個選ぶ連結の辞書順最小を返す。
